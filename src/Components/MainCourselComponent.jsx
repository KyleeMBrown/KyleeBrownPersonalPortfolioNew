        ...
        >
            {data.map((data, index)=>(
                <Carousel.Slide key={index}>
                    <div style={{backgroundImage:`url("${data.url}")`}} className="bg-cover font-sans bg-center h-full flex items-center justify-center">
                     {data.title ? 
                     <div className="flex items-center text-[1.25em] text-[#355748] justify-center backdrop-blur-lg bg-[#cdeaeb65] p-[1em] flex-col">
                        <p className="italic ">Aspiring...</p>
                        <br></br>
                        <h2 className="font-bold text-[2em] max-[760px]:text-[0.75em]">{data.title}</h2>
                        <br></br>
